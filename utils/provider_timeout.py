...
zen-MCP hang durable-fix — SOL-338 Layer 2 INNER layer (2026-06-18).
...
KNOWN LIMITATION (disclosed; follow-up tracked)
    ...
    returns or the OS TCP timeout (~15-30 min). The OUTER supervisor
    (scripts/zen_supervisor.py) hard-kills the whole server process group as the
    backstop that DOES reap the leaked FD; the true in-source reap is native
    per-provider httpx/SDK transport timeouts — the MANDATORY follow-up, tracked
    on PR #7's line, not this stopgap.
"""
...
async def generate_content_with_timeout(provider, **kwargs):
    ...
    try:
        ...
        logger.error(
            ...
            "leak until the SDK socket returns (see module docstring); the outer "
            "supervisor reaps it via process-group kill.",
