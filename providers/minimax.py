...
class MiniMaxModelProvider(OpenAICompatibleProvider):
    ...
    MODEL_CAPABILITIES = {
        "MiniMax-M2.7": ModelCapabilities(
            ...
            description="MiniMax M2.7 (200K context, 128K output) - Self-evolving agentic SOTA rivaling Claude Opus/GPT-5 on long-chain tool use. Direct-China (api.minimax.io) = LOW-sensitivity only; bare `minimax` routes to US-brokered OpenRouter M2 instead (GENESIS-096).",
        ...
        "MiniMax-M2-direct": ModelCapabilities(
            provider=ProviderType.MINIMAX,
            model_name="MiniMax-M2-direct",
            friendly_name="MiniMax AI (M2 direct)",
            context_window=204_800,
            max_output_tokens=131_072,
            supports_extended_thinking=True,
            supports_system_prompts=True,
            ...
            supports_temperature=True,
            temperature_constraint=RangeTemperatureConstraint(0.0, 2.0, 1.0),
            description="MiniMax M2 (200K context) - Original agent-native coding model; 230B MoE / 10B active. DIRECT-CHINA (api.minimax.io) = LOW-sensitivity ONLY. Renamed -direct 2026-06-27 (GENESIS-096) so the bare `minimax`/`m2`/`minimax-m2` seat resolves to US-brokered OpenRouter (minimax/minimax-m2, Western-inferenced, MODERATE-eligible).",
            aliases=["m2-direct", "minimax-coding"],
